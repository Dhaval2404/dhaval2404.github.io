...
const experienceYearsLabel = `${APP_CONFIG.YEARS_OF_EXPERIENCE} years`;
...
export default function HeroSection() {
  return (
    ...
    >
      <div>
        <Reveal>
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-green-200 bg-green-100 px-3 py-1 dark:border-green-500/30 dark:bg-green-500/10">
            <span className="relative flex size-3" aria-hidden="true">
              <span className="absolute inline-flex size-full animate-ping rounded-full bg-green-400 opacity-75" />
              <span className="relative inline-flex size-3 rounded-full border-2 border-green-100 bg-green-500 dark:border-slate-950" />
            </span>
        ...
        <Reveal delay={0.1}>
          ...
          <p className="mt-6 max-w-2xl text-lg font-medium text-slate-body">
            {experienceYearsLabel} building secure and scalable mobile
            platforms. Currently modernizing critical payment systems for{" "}
      ...
      <Reveal delay={0.3} y={32} className="w-full lg:w-auto">
        <div className="relative flex w-full flex-1 justify-center lg:justify-end">
          ...
          <div className="relative h-[400px] w-full max-w-[500px] overflow-hidden rounded-2xl border border-border-light bg-surface-light shadow-2xl">
            ...
            <div className="space-y-2 p-6 font-mono text-sm text-slate-600 dark:text-slate-300">
              <p>
              ...
              <p className="pl-4">
                <span className="text-purple-600 dark:text-purple-400">const val</span>{" "}
                <span className="text-indigo-600 dark:text-indigo-400">experience</span> ={" "}
                <span className="text-green-700 dark:text-green-400">&quot;{experienceYearsLabel}&quot;</span>
              </p>
              <p className="pl-4">
                <span className="text-purple-600 dark:text-purple-400">val</span>{" "}
                <span className="text-indigo-600 dark:text-indigo-400">focus</span> ={" "}
                <span className="text-blue-600 dark:text-blue-400">listOf</span>(
                <span className="text-green-700 dark:text-green-400">&quot;Mobile&quot;</span>,{" "}
                <span className="text-green-700 dark:text-green-400">&quot;Backend&quot;</span>)
              </p>
              <p className="pl-4">
                <span className="text-purple-600 dark:text-purple-400">val</span>{" "}
                <span className="text-indigo-600 dark:text-indigo-400">strengths</span> ={" "}
                <span className="text-blue-600 dark:text-blue-400">listOf</span>(
              </p>
              {["Architecture", "Scalability", "Delivery", "Leadership"].map(
                (strength, index) => (
                  <p key={strength} className="pl-8 text-green-700 dark:text-green-400">
                    &quot;{strength}&quot;{index < 3 ? "," : ""}
                  </p>
                ),
              )}
