        ...
        <div className="grid grid-cols-1 md:grid-cols-2 items-stretch gap-8 lg:grid-cols-2">
          {experiences.map((experience) => (
            <article
              ...
              className={`flex h-full flex-col overflow-hidden rounded-2xl border border-border-light bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-xl ${experience.wide ? "lg:col-span-2" : ""}`}
            >
              <div className="mb-6 flex flex-col items-start justify-between gap-2 sm:flex-row">
                ...
                <p className="whitespace-nowrap rounded-full border border-slate-100 bg-slate-50 px-3 py-1 text-left text-sm font-medium text-slate-500 sm:text-right">
                  {experience.period}
                </p>
              </div>
              <div className="grow">
                <p className="mb-4 text-slate-600">{experience.summary}</p>
                <ul
